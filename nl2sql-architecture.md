...
**结论**：严肃的 NL2SQL 查询必须遵循 OLTP/OLAP 分离原则。复杂中间层（SemanticPlan、CapabilityCatalog 等）是 SaaS 平台面对未建数仓客户的兜底方案，不是工程最优解。
...
**选择**：**S3/对象存储上的 Lakehouse**，优先 Iceberg。开放表格式，原生分区裁剪，与 DuckDB/Spark/Trino 等查询引擎同源。
...
```python
# 脚本层：CTE 包装
def inject_filter(llm_sql: str, user_id: str) -> str:
    """将 LLM 生成的 SQL 包装在带过滤的 CTE 中"""
    return f"""
    WITH filtered_data AS (
        SELECT * FROM data WHERE user_id = '{user_id}'
    )
    {llm_sql.replace('data', 'filtered_data')}
    """
```
