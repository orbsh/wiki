...
NL2SQL 不是要上线的功能，是要持续运转的系统。核心是**反馈循环**和**增长飞轮**：

```
用户查询 → SQL 执行 → 结果反馈 → Schema/提示词优化 → 更好的查询
    ↑                                                        │
    └────────────────────────────────────────────────────────┘
```

- **反馈循环**：每次查询都是训练信号——哪些字段常被问到、哪些 SQL 模式常出错、哪些业务定义需要补充到 Skill
- **增长飞轮**：更多数据 → 更多查询 → 更多反馈 → 更好的 Schema/提示词 → 更多人用
- **流水线**：不是一次性构建，是持续演进的管线。基础设施投入是为数据业务本身，不是为 NL2SQL
...
| **湖仓一体（Iceberg）** | ✅ **首选** | Parquet + 元数据层，开销极小。S3 原生，无限扩展。|
| 湖仓一体（Delta Lake）| ✅ 备选 | 基于文件事务日志，无需 catalog 服务。但生态封闭度高于 Iceberg。|

**选择**：**S3/对象存储上的 Lakehouse**，优先 Iceberg。

- **Iceberg 不重**：它只是 Parquet + 元数据。说 Iceberg 重的人，多半是用 Spark/Flink 在查——重的是引擎，不是表格式。凡是只支持通过 Spark/Flink Java 计算平台查 Iceberg 的方案，都不合格，给钱都不能用。
- **小数据也适用**：2k 条不变的数据直接用 CSV 也行，DuckDB 能直接查。但 Parquet 体积小、列式高效，S3 上直接存 Parquet + DuckDB 查询是更优路径。
- **Schema evolution** 由表格式原生支持，但宽表 schema 变更时需同步更新 Skill 提示词。
...
| 工具 | 结论 | 原因 |
|------|------|------|
| Spark/Flink（Java/Scala）| ❌ 拒绝 | JVM 体系，启动慢、资源重。查 Iceberg 用这些引擎是重的根源。|
| Polars | ✅ 读层备选 | 轻量、快速，但 SQL 支持有限。适合 DataFrame 操作。|
| DuckDB | ✅ **读层首选** | ~100MB，功能极其丰富。原生 SQL、Python 绑定、直接查 S3 上的 Parquet/Iceberg。|
| `deltalake` Python | ✅ 写层 | 基于 Rust、经过验证。AI 永远不触碰写入路径。|

**选择**：**DuckDB 读取 + `deltalake` 写入**。职责分离：技能的 `run.py` 拥有写入逻辑；AI 只生成 SELECT 查询。

- **DuckDB 是正确的查询引擎**：~100MB 安装包，功能极其丰富。100M 级别的软件现在根本不是事儿，装个 Python 差不多体积。别被 Spark/Flink 的重量吓到——那是 Java 计算平台的问题，不是 OLAP 的问题。
- **高并发是所有 OLAP 的共同短板**：ClickHouse 高并发也不行，这是列式存储的固有限制。NL2SQL 场景是低频分析查询，不是高并发事务，正好匹配。
