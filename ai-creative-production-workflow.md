...
这个架构的本质是**降维**——将 AI 从高自由度的几何空间中解放出来：

- **文字/代码**（一维离散空间）：LLM 生成一个 token 只需在几万词字典里做一次选择题，表达结构是线性的
- **图像/视频**（连续像素空间）：生成一帧 1920×1080 画面需同时决定近 600 万个 RGB 变量，加时间轴后前后帧无物理约束——手指多一根、穿墙等物理崩塌皆源于此
- **3D 模型**（高维拓扑空间）：数万顶点 + 骨骼关节 + 权重网格 + 着色器，自由度指数级爆炸。直接让 AI "凭空生成"这些底层数据，无法保证"张嘴时嘴角不撕裂"、"抬手时肩膀不坍塌"

管线的解耦策略：AI 只在低维 JSON 参数空间操作（`{"height": 0.8, "weight": 0.9}`），3D 引擎（MPFB2 + Rigify）内部已写死解剖学逻辑和 IK/FK 数学公式，拿到参数后确定性地展开为高维几何。**AI 不画 3D，才是这个架构的正确之处。**

一致性来自持久化：资产一旦烘焙就是固定的，Cycles 渲染一万帧角色脸也不会变。直接 AI 生成每帧重新计算概率，必然漂移。
