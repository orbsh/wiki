...
传统的"多语种持久化（Polyglot Persistence）"方法（为 KV、图、关系需求使用不同数据库）制造运维复杂性和数据碎片化。SurrealDB 将这些模型统一到一个引擎中。→ Redis 替代论证见 [Redis 批判](redis-critique.md)，湖仓分析见 [Lakehouse 研究](lakehouse-research.md)。
...
| 执行边界 | DB 内（逻辑被迫搬到应用层编排） | 进程内（Polars 读入数据） / 数据本地性（Spark 就近计算） | DB 内（计算下推，无数据搬运） |
...
三者不是"纯声明式 vs 一体化"的二元对立，而是**组合位置和通用性的取舍**：SQL 不能组合（字符串），逻辑被迫搬到应用层；Polars/Spark 能组合（宿主语言），通用性强但语法空间切换有摩擦（Polars 需读入数据，Spark 靠数据本地性就近计算但仍是独立计算引擎）；SurQL 能组合（一体化语法），无摩擦、逻辑在 DB 内，代价是运行环境限于 DB，但通过 HTTP 和 Wasm 插件可按需扩展外部能力。
...
- **防御**：SurQL 支持 **SQL 兼容语法模式**用于传统查询。这为 AI 边助生成基础 CRUD 提供了安全的回退，而命令式逻辑保留在 SurQL 中。
...
- **OLAP（分析）**：SurrealDB 不应在重度数据处理中竞争。OLAP 的最终答案是**湖仓一体**（Lakehouse）。SurrealDB 作为高频实时"上下文状态机"或"AI 代理记忆"，通过 CDC 将历史数据流式传输到湖中。这将**可变状态**（由 SurrealDB 处理）与**静态分析**（由湖仓处理）分离。→ 详细选型见 [Lakehouse 研究](lakehouse-research.md)。
