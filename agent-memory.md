...
| **夜间定时** | 用户不活跃时（如凌晨）后台触发压缩 | 不影响用户体验，利用闲置算力 | 需要定时任务基础设施 |
| **长期闲置** | session 长时间无活动时触发，可结合 cache 到期时间续期 | 防止 cache 过期失效，保持热缓存 | 需要监听 session 活跃状态 |
| **主题转换** | 检测到对话主题切换时触发 | 按语义边界压缩，摘要更清晰 | 需要主题检测能力（图谱化记忆） |
...
| **夜间定时** | 用户不活跃时 | 定时任务 | 同 Prefix Checkpoint |
| **长期闲置** | session 闲置 + cache 快过期 | 流式计算 | 续期 checkpoint |
| **主题转换** | 语义边界 | 图谱聚簇 | 按主题分段的摘要 |
